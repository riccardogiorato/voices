...
async function bench(label, options) {
  ...
  console.log(`\n=== ${label} ===`)
  console.log(`  Session create: ${createMs.toFixed(0)}ms`)
...
console.log('=== LLVC WASM Benchmark: Thread/optimization comparison ===')

await bench('WASM default (1 thread)', { numThreads: 1 })
await bench('WASM 4 threads', { numThreads: 4 })
await bench('WASM 8 threads', { numThreads: 8 })
await bench('WASM SIMD only (no threads)', { numThreads: 1, enableSimd: true })
